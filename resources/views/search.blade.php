    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            ...
            <div class="sort">
                    ...
                    <a href="{{route('index','asc')}}">Возрастанию</a> ||
                    <a href="{{route('index','desc')}}">Убыванию</a>
                </div>
            @if (count($all)>0)
            @foreach ($all as $item)
            <div class="row main_row">   
                <div class="col-md-3 foto">
                        <a href="{{route('view',$item->id)}}"><img src="{{asset('images/'.$item->image_names)}}" class="main_foto img-thumbnail" alt="mainFoto"></a>    
                </div>
                <div class="col-md-7 middle_block">
                    <div>
                            <h4><a href="{{route('view',$item->id)}}" class="title">{{ $item->title }}</a></h4>
                            <div class="grey">{{$item->rubric}}</div>
                    </div>
                    <div>
